...
- **Default feature**: _(none — tickets in this repo don't roll up under a single feature)_

If `exponential workspaces set-default syntrofi` is set on the local CLI, `--workspace` can be omitted in most commands. `--product agent-skills` is always required.
...
- **Create a ticket**: `exponential tickets create --product agent-skills --type <TYPE> --status <STATUS> -t "<title>" -b "<body>" [--feature <feature-cuid>] [--epic <epic-cuid>] --json`. Use a heredoc for multi-line bodies.
- **Read a ticket**: `exponential tickets get <ticket-cuid> --json` (returns dependencies, actions, and comments).
- **List tickets**: `exponential tickets list --product agent-skills [--status <STATUS>] [--type <TYPE>] [--feature <cuid>] [--assignee <user-id>] --json`. Status filtering is server-side — prefer that to client-side filtering.
...
- **Create a feature (for PRDs)**: `exponential features create --product agent-skills -n "<name>" -d "<description>" --vision "<target outcome>" --status DEFINED --json`.
...
```bash
exponential tickets list --product agent-skills --status BACKLOG --json          # needs-triage
exponential tickets list --product agent-skills --status NEEDS_REFINEMENT --json # needs-info
exponential tickets list --product agent-skills --status READY_TO_PLAN --json    # ready-for-agent
exponential tickets list --product agent-skills --status BLOCKED --json          # ready-for-human
```

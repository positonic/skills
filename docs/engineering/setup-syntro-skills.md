...
```bash
npx skills add mattpocock/skills --skill=setup-syntro-skills
```

```bash
npx skills update setup-syntro-skills
```

[Source](https://github.com/mattpocock/skills/tree/main/skills/engineering/setup-syntro-skills)

## What it does

`setup-syntro-skills` teaches one repo how the engineering skills should behave in it — where issues live, what the triage labels are called, and where the domain docs sit — and records those answers as **config** the other skills read.
...
You invoke this by typing `/setup-syntro-skills` — the agent won't reach for it on its own.

...
`to-tickets` publishes into your issue tracker, so run `/setup-syntro-skills` once in this repo first — it configures the tracker and its triage label vocabulary. On a real tracker it applies the ready-for-agent label as it publishes.

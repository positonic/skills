...
`triage` reads and writes your issue tracker, so run `/setup-syntro-skills` once in this repo first — it configures the tracker and the label mapping. The role names above are **canonical** — the actual label strings in your tracker may differ, and that mapping is what setup provides. The config also decides whether external PRs count as a request surface, and who counts as external.

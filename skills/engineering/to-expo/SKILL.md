...
- Explicit instructions to:
  - before any Ticket work, create an **isolated git worktree** for the batch, based on `origin/<featureBase>` (featureBase from `docs/agents/git-flow.md`; default `main`), so the user's primary checkout stays put on its current branch and never inherits the batch's dirty state. Detect the repo's worktree naming convention from `git worktree list` (e.g. sibling dirs like `<repo>-<feature>`); else fall back to a sensible default. Create it detached at the base and let `/start-ticket` cut each Ticket's branch inside the worktree,
  - **auto-wire the worktree's local setup before coding**: symlink local-only required files the worktree lacks (gitignored/untracked files present in the primary tree — env files, local config, credentials), install dependencies inside the worktree with the detected (lockfile-driven) package manager, and run code generation after any schema/codegen-source edit so typecheck passes locally,
  - **be robust to the shell working directory resetting between tool calls** — drive git with `git -C <worktree> …`, use absolute paths under the worktree for file edits, and ensure CWD-bound skills (`/start-ticket`, `/ship-ticket`) run against the worktree (or replicate their steps targeting it explicitly),
...
0. **Set up an isolated worktree first.** Do all batch work in a dedicated git worktree so the user's primary checkout stays on its current branch and never shares this batch's dirty state.
   - Find the base: read `featureBase` from `docs/agents/git-flow.md` (default `main`). Fetch first: `git fetch origin`.
   - Pick the worktree path using the repo's existing convention — inspect `git worktree list` for the pattern in use (commonly a sibling dir like `../<repo>-<feature>`); if there's no convention, fall back to a sensible default such as `../<repo>-<feature>`.
   - Create it detached at the base so no branch is checked out yet: `git worktree add --detach <worktree> origin/<featureBase>`. `/start-ticket` then cuts each Ticket's branch *inside* the worktree.
   - **Wire up local-only required files the worktree lacks.** Worktrees do not copy gitignored/untracked files. Find files present in the primary tree but ignored by git (env files, local config, credentials — e.g. `.env`, `.env.local`) and **symlink** each into the worktree at the same relative path. They remain gitignored, so secrets are never committed — confirm with `git -C <worktree> check-ignore <file>` (it should print the path).
   - **Install dependencies inside the worktree** with the detected, lockfile-driven package manager. Do a real install — do NOT symlink `node_modules` from the primary tree, as that breaks codegen/binary tools (e.g. `prisma generate`).
   - **Run code generation after any schema/codegen-source edit** (e.g. `prisma generate`) so typecheck passes locally *without* applying a migration. Applying the migration is a human step: write the schema change, regenerate the client, then ask the human to run the migration before shipping a schema-bearing Ticket.
   - **Be robust to the shell working directory resetting between commands.** Don't assume `cd` persists across tool calls — drive git with `git -C <worktree> …` and use absolute paths under the worktree for file edits. CWD-bound skills (`/start-ticket`, `/ship-ticket`) act on the *current* directory, so ensure they run against the worktree (or replicate their steps targeting the worktree explicitly).
1. **Start** the Ticket against the worktree: `/start-ticket <cuid>` — fetches the body, transitions to `IN_PROGRESS`, checks out (or creates) its branch inside the worktree, and writes `.exponential/current-ticket`. (If invoking it can't be scoped to the worktree, replicate its steps there: `git -C <worktree> checkout -b <branchName> origin/<featureBase>` and write the marker file under the worktree.)
2. Read the Ticket body and acceptance criteria.
3. Implement the slice end-to-end inside the worktree (absolute paths under `<worktree>`). Run codegen after any schema/codegen-source edit. Verify the acceptance criteria locally.
4. **Ship** the Ticket from the worktree: `/ship-ticket` (no arg — it auto-detects from the marker file). Pre-ship checks run; a PR opens (or extends, if you stacked on the same branch); the Ticket transitions to `QA`; `ticket.prUrl` is set. The `QA`→`DONE` promotion happens only once the PR actually merges — so reaching `QA` does **not** make the Ticket's code available to later Tickets. For schema-bearing Tickets, confirm the human has run the migration before shipping.
5. **Merge a Ticket's PR to `main` before starting any Ticket that depends on it.** `QA` is only a status; the blocker's *code* lives on its branch, not in `main`, until its PR merges. Each new Ticket's branch is cut off `origin/<featureBase>` inside the worktree, so a freshly-merged `main` is what both keeps dependent Tickets compiling and prevents stacked PRs. Independent Tickets (no shared blocker) may proceed in parallel off `main`. Never base ticket work on an unmerged feature branch, and never leave a tower of stacked branches.

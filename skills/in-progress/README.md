...
- **[pr-review](./pr-review/SKILL.md)** — Adversarial staff-engineer-level PR review focused on production risk: correctness, security, performance, architecture, missing tests. High signal, low noise.
- **[pr-fix-all](./pr-fix-all/SKILL.md)** — Address open review comments on a PR end-to-end: fix → commit per-finding → push → prepend `✅ Addressed in <sha>` to each comment → optionally resolve threads. Pairs with `/pr-review`.

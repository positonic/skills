...
- **[to-bug](./skills/engineering/to-bug/SKILL.md)** — File one known defect as a `BUG` ticket in [Exponential](https://www.exponential.im) in a single shot: product and workspace resolved from the repo you're standing in, expected/actual/repro captured from the failing output already on screen. Never invents a repro — files as `NEEDS_REFINEMENT` instead.
...
- **[ship-ticket](./skills/engineering/ship-ticket/SKILL.md)** — Ship one Exponential Ticket: commit leftovers, run pre-ship checks, open or extend a PR, link it back to the Ticket, transition to `QA`, then wait for the repo's automated reviewer and apply its findings. Merges only with `--merge`.
- **[ship-this](./skills/engineering/ship-this/SKILL.md)** — Ship the current working copy end-to-end with zero hand-holding: branch if needed, commit, push, open a PR against the right base, wait for whichever automated reviewer the repo has (PR-Agent, CodeRabbit, or a local `/pr-review`) and apply its findings, wait for CI, and squash-merge. The ticket-free sibling of `/ship-ticket`.
- **[cleanup](./skills/engineering/cleanup/SKILL.md)** — Tear down after work that landed: remove the git worktree, delete its branch, promote its Exponential Tickets from `QA` to `DONE`, and close with the all-clear — an explicit statement of whether anything is still in flight, so you know whether you can close the tab. `/ship-this` and `/ship-ticket` hand off to it once a merge is real; `--all` sweeps the whole repo.
...
- **[setup-merge-hook](./skills/engineering/setup-merge-hook/SKILL.md)** — Scaffold the GitHub Action that reacts to a merge into the deploy trigger: promote Exponential Tickets from `QA` to `DONE`, tick the merged scope's feature requirements as met, or both.

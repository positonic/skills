...
- **[cleanup](./skills/engineering/cleanup/SKILL.md)** — Tear down after work that landed: remove the git worktree, delete its branch, promote its Exponential Tickets from `QA` to `DONE`, and close with the all-clear — an explicit statement of whether anything is still in flight, so you know whether you can close the tab. `--all` sweeps the whole repo.
